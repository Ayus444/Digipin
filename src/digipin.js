...
function isValidDigiPin(pin) {
  if (typeof pin !== "string") return false;
  const cleaned = pin.replace(/-/g, "").toUpperCase();
  if (cleaned.length !== 10) return false;

  const allowed = new Set(DIGIPIN_GRID.flat());
  if (![...cleaned].every((c) => allowed.has(c))) return false;
